...
export default function DealDetailsPage() {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      const businessId = deal.proposingBusinessId;
      const athleteName = deal.athleteName;

      batch.update(dealDocRef, { status: 'accepted' });
      
      batch.set(doc(notificationsRef), {
        recipientId: businessId,
        message: `${athleteName} has accepted your deal proposal.`,
        link: `/deal-details/${deal.id}`,
        type: 'deal_accepted', // NEW: Specific notification type
        read: false,
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      batch.update(dealDocRef, { status: 'rejected' });

      batch.set(doc(notificationsRef), {
        recipientId: businessId,
        message: `${athleteName} has rejected your deal proposal.`,
        link: `/deal-details/${deal.id}`,
        type: 'deal_rejected', // NEW: Specific notification type
        read: false,
        timestamp: serverTimestamp(),
      });
  ...
  const handleRevoke = async () => {
    ...
    try {
      ...
      const dealDocRef = doc(db, 'deals', deal.id);
      const notificationsRef = collection(db, 'notifications');
      const recipientId = deal.athleteId;
      const businessName = deal.proposingBusinessName;

      batch.update(dealDocRef, { status: 'revoked' });

      batch.set(doc(notificationsRef), {
        recipientId: recipientId,
        message: `${businessName} has revoked their deal proposal.`,
        link: `/deal-details/${deal.id}`,
        type: 'deal_revoked', // NEW: Specific notification type
        read: false,
        timestamp: serverTimestamp(),
      });
